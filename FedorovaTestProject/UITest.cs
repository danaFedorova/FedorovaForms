...
namespace FedorovaForms
{
    internal class UITest
    {
        ...
        string PathTestingApp = @"C:\Users\antailyaqwer\source\repos\FedorovaForms\FedorovaForms\bin\Debug\net6.0-windows\FedorovaForms.exe";
        /// <summary>время мс допустимой задержки</summary>
        int M = 5000;
        //текст внутри элементов управления
        const string registrationTitleString = "Form1";
        ...
        public void T_001_RegisterForm()
        {
            ...
            using (var automation = new UIA3Automation())
            {
                ...
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.EmptyLogin, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.EmptyLogin, errorLabel.Text);
                ...
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.EmptyPassword1, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.EmptyLogin, errorLabel.Text);
                ...
                retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.EmptyPassword2, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.EmptyPassword2, errorLabel.Text);
                ...
                retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.DifferentPasswords, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                ...
                retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.SameLoginPassword, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.SameLoginPassword, errorLabel.Text);
                ...
                retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.PasswordLess10Chars, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.PasswordLess10Chars, errorLabel.Text);
                ...
                retry = Retry.WhileException(() => {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.PasswordNoNumber, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.PasswordNoNumber, errorLabel.Text);
                ...
                retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.PasswordNoExtraChar.Replace("&&", "&"), errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.PasswordNoExtraChar.Replace("&&", "&"), errorLabel.Text);
                ...
                retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.PasswordNoUpperChar, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.PasswordNoUpperChar, errorLabel.Text);
                ...
                retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.LoginForbidden, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.LoginForbidden, errorLabel.Text);
        ...
        public void T_002_RegisterForm()
        {
            ...
            using (var automation = new UIA3Automation())
            {
                ...
                var retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.NoConnectionDB, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.NoConnectionDB, errorLabel.Text);
        ...
        public void T_003_RegisterForm()
        {
            ...
            using (var automation = new UIA3Automation())
            {
                ...
                var retry = Retry.WhileException(() =>
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.LoginAlreadyExists, errorLabel.Text);
                }, TimeSpan.FromMilliseconds(M));
                if (!retry.Success)
                {
                    Assert.AreEqual(FedorovaForms.ExceptionStrings.LoginAlreadyExists, errorLabel.Text);
